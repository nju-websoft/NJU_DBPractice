...
static ssize_t readn(int sockfd, char *buf, int len)
{
  ...
  while (nleft > 0) {
    ...
    if (n < 0) {
      WSDB_LOG(net, readn, "ERROR reading from socket");
      return -1;
    } else if (n == 0) {
      WSDB_LOG(net, readn, fmt::format("connection to {} closed", sockfd));
      return -1;
    }
    nleft -= n;
    buf += n;
...
static ssize_t writen(int sockfd, char *buf, int len)
{
  ...
  while (nleft > 0) {
    ...
    if (n < 0) {
      WSDB_LOG(net, writen, "ERROR writing to socket");
      return -1;
    } else if (n == 0) {
      WSDB_LOG(net, writen, fmt::format("connection to {} closed", sockfd));
      return -1;
    }
    nleft -= n;
    buf += n;
