...
select s_i_id, s_w_id from stock where s_i_id > 249 and s_i_id < 256;
...
select s_i_id, s_w_id from stock where s_w_id = 10 and s_i_id > 399 and s_i_id < 406;
...
select s_i_id, s_quantity from stock where s_quantity = 85 and s_i_id > 499 and s_i_id < 511;

-- 6. Test range delete on quantity (should use stock_sqty_idx)
delete from stock where s_quantity > 95 and s_quantity < 98;
select s_i_id, s_quantity from stock where s_quantity > 94 and s_quantity < 99;

-- 7. Test delete using order count index
delete from stock where s_order_cnt = 75 and s_i_id > 600 and s_i_id < 610;
select s_order_cnt, s_i_id from stock where s_order_cnt = 75 and s_i_id > 599 and s_i_id < 611;
...
select s_order_cnt, s_remote_cnt, s_ytd from stock where s_order_cnt = 50 and s_remote_cnt = 25 and s_ytd > 4.9 and s_ytd < 6.1;

-- 9. Test delete by ytd range
delete from stock where s_ytd > 9.0 and s_ytd < 9.5;
select s_ytd, s_i_id from stock where s_ytd > 8.9 and s_ytd < 9.6;

-- 10. Test complex multi-condition delete
delete from stock where s_w_id = 15 and s_quantity > 80 and s_order_cnt > 60;
select s_w_id, s_quantity, s_order_cnt from stock where s_w_id = 15 and s_quantity > 79 and s_order_cnt > 59;
