...
select s_i_id, s_w_id from stock where s_i_id > 1599 and s_i_id < 1606 order by s_i_id;
...
select s_i_id, s_w_id from stock where s_w_id = 88 order by s_i_id;
select s_i_id, s_w_id from stock where s_w_id = 60 and s_i_id > 1899 and s_i_id < 1906 order by s_i_id;
...
select s_i_id, s_quantity, s_dist_02 from stock where s_quantity = 10 and s_i_id > 2199 and s_i_id < 2211 order by s_i_id;
...
select s_order_cnt, s_remote_cnt, s_ytd, s_dist_03 from stock where s_order_cnt = 75 and s_remote_cnt = 35 and s_ytd > 4.9 and s_ytd < 6.1 order by s_i_id;
...
select s_ytd, s_dist_04 from stock where s_ytd > 0.9 and s_ytd < 2.1 order by s_i_id;
...
select s_remote_cnt, s_ytd, s_order_cnt, s_dist_05 from stock where s_remote_cnt = 45 and s_ytd > 2.9 and s_order_cnt > 19 and s_order_cnt < 41 order by s_i_id;
...
select s_i_id, s_w_id, s_dist_06 from stock where s_w_id = 80 and s_i_id > 2499 and s_i_id < 2511 order by s_i_id;
...
select s_quantity, s_dist_07 from stock where s_dist_07 = 'SCN001' order by s_i_id;
...
select s_i_id, s_w_id, s_dist_08 from stock where s_i_id > 2799 and s_w_id = 90 order by s_i_id;

-- 19. Test large range update
update stock set s_dist_09 = 'LRG001' where s_i_id > 2900 and s_i_id < 2950;
select s_i_id, s_dist_09 from stock where s_i_id > 2899 and s_i_id < 2951 order by s_i_id;
...
select s_i_id, s_quantity, s_dist_10 from stock where s_quantity > 49 and s_quantity < 71 and s_i_id > 2999 and s_i_id < 3101 order by s_i_id; 